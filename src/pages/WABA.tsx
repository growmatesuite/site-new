...
import {
  ...
  Shield,
  Zap,
} from "lucide-react"; // Assuming lucide-react is installed as per previous files
...
declare global {
  interface Window {
    fbAsyncInit: () => void;
    FB: any; // Type declaration for Facebook SDK object
  }
}
...
const formatPhoneNumber = (value: string): string => {
  ...
  if (limitedNumbers.length <= 2) return limitedNumbers; // Código do país
  if (limitedNumbers.length <= 4)
    return `${limitedNumbers.slice(0, 2)} ${limitedNumbers.slice(2)}`; // Código + DDD
...
export default function WABA() {
  ...
  useEffect(() => {
    ...
    window.fbAsyncInit = function () {
      if (window.FB) {
        window.FB.init({
          appId: "968258385042083", // Replace with your App ID
          cookie: true,
          xfbml: true,
          version: "v24.0",
        });
    ...
    let checkFB: NodeJS.Timeout | null = null;
    let timeout: NodeJS.Timeout | null = null;

    if (typeof window !== "undefined") {
      // If script already loaded and FB exists, initialize directly
      if (window.FB) {
        window.FB.init({
          appId: "968258385042083",
          cookie: true,
          xfbml: true,
          version: "v24.0",
        });
        ...
        // Wait for script to load and call fbAsyncInit
        checkFB = setInterval(() => {
          if (window.FB) {
            if (checkFB) clearInterval(checkFB);
            // If fbAsyncInit wasn't called automatically, call it
  ...
  const loginWithFacebook = () => {
    if (!fbReady || !window.FB) {
      setStatus("Facebook SDK not ready. Please wait...");
      return;
    }
    ...
    // Telefone está disponível aqui
    console.log("Telefone do usuário:", cleanPhone);
    ...
    window.FB.login(
      function (response: any) {
        if (response.authResponse) {
          ...
          console.log("ephemeral code: ", code);

          fetch(
            "https://n8n.growmate.io/webhook/backoffice-whatsapp-business-onboarding",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Basic YWRtaW46cnQ4a04tWCFkY0pXSnlDeFJDOXdAPCRJaUs5fjM2OXk=`,
              },
              body: JSON.stringify({
                code: code,
                phone: cleanPhone,
          ...
          window.FB.api(
            ...
            function (response: any) {
              ...
              // Handle user data, send to your backend for further processing
            },
          );
        } else {
      ...
      {
        config_id: "1916196495487463",
        response_type: "code",
        override_default_response_type: true,
        extras: {
          setup: {
            success_uri:
              "https://n8n.growmate.io/webhook/meta-business-success-onboarding",
          },
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Conecte seu WhatsApp Business API (WABA) com a plataforma Growmate de forma segura e oficial."
        />
      </Helmet>
        ...
        <main className="flex-grow pt-32 pb-20">
          <Container size="lg">
            <div className="flex flex-col items-center justify-center text-center max-w-4xl mx-auto mb-16">
              ...
              >
                Conecte sua conta do WhatsApp Business API (WABA) à nossa
                plataforma e desbloqueie o poder da automação oficial.
              </motion.p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-stretch max-w-5xl mx-auto">
                  ...
                  <ul className="space-y-6">
                    <li className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-400">
                          Garanta a entrega das suas mensagens sem riscos de
                          banimento por uso de automações não oficiais.
                        </p>
                      </div>
                    </li>
                    <li className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-400">
                          Envie notificações ativas e aprovadas pela Meta para
                          seus clientes.
                        </p>
                      </div>
                    </li>
                    <li className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-400">
                          Atenda milhares de clientes simultaneamente com
                          múltiplos agentes.
                        </p>
                      </div>
                    </li>
              ...
              >
                <Card
                  glowColor="green"
                  className="h-full p-8 border-gray-700 bg-black flex flex-col justify-center relative overflow-hidden group"
                >
                  ...
                  <div className="relative z-10 text-center">
                    ...
                    <p className="text-xs text-red-500 mt-4">{status}</p>
                  </div>
                </Card>
              </motion.div>
            </div>
